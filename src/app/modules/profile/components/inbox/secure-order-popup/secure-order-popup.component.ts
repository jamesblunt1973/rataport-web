...
@Component({
  selector: 'app-secure-order-popup',
  templateUrl: './secure-order-popup.component.html',
  styleUrls: ['./secure-order-popup.component.scss']
})
export class SecureOrderPopupComponent implements OnInit {
  ...
  constructor(public dialogRef: MatDialogRef<SecureOrderPopupComponent>,
  ...
  orderCancelAfterShipping() {
    ...
    if (this.orderCancelAfterShippingForm.valid) {
      ...
      if (this.cancelAfterShippingImages.length > 0) {
        this.sendAdd = true;
        this._profile.secureOrderCancelAfterShipping(this.orderCancelAfterShippingForm.value).subscribe(
          result => {
            if (result['success']) {
              this.close();
  ...
  orderCancelBeforShipping() {
    ...
    if (this.orderCancelBeforShippingForm.valid) {
      this.sendAdd = true;
      this._profile.secureOrderCancelBeforeShipping(this.orderCancelBeforShippingForm.value).subscribe(
        result => {
          if (result['success']) {
            this.close();
            this.openSnackBar('اطلاعات با موفقیت ارسال شد', 'بستن');
  ...
  orderCReceiveGoodsByBuyer() {
    ...
    if (this.agreeCtrl1.value) {
      if (this.orderReceiveGoodsByBuyerForm.valid) {
        ...
        this.sendAdd = true;
        this._profile.secureOrderReceiveGoodsByBuyer(this.orderReceiveGoodsByBuyerForm.value).subscribe(
          result => {
            if (result['success']) {
              this.close();
              this.openSnackBar('اطلاعات با موفقیت ارسال شد', 'بستن');
  ...
  orderShippingBySeller() {
    ...
    if (this.orderShippingBySellerForm.valid) {
      ...
      if (this.finalImage !== '' && this.cancelAfterShippingImages.length > 0) {
        this.sendAdd = true;
        this._profile.secureOrderShippingBySeller(this.orderShippingBySellerForm.value).subscribe(
          result => {
            if (result['success']) {
              this.close();
              this.openSnackBar('اطلاعات با موفقیت ارسال شد', 'بستن');
  ...
  orderReceiveGoodsBySeller() {
    ...
    if (this.agreeCtrl2.value) {
      if (this.orderReceiveGoodsBySellerForm.valid) {
        ...
        this.sendAdd = true;
        this._profile.secureOrderReceiveGoodsBySeller(this.orderReceiveGoodsBySellerForm.value).subscribe(
          result => {
            if (result['success']) {
              this.close();
              this.openSnackBar('اطلاعات با موفقیت ارسال شد', 'بستن');
