...
@Component({
  selector: 'app-sidebar-brands',
  templateUrl: './sidebar-brands.component.html',
  styleUrls: ['./sidebar-brands.component.scss']
})
export class SidebarBrandsComponent implements OnInit, OnDestroy, AfterViewInit {

  public property: any = [];

  @Output() public filterOptionsBrands: EventEmitter<any> = new EventEmitter<any>();
  ...
  sendFilters() {
    this.filterOptionsBrands.emit(this.brandsFilters);
  }
  ...
  oneLevelSelected(event, name) {
    ...
    if (name === 'nofilter') {
      this.brandsFilters.categoryID = event.value;
      this.brandsFilters.categoryLevel = 1;
      this.sendFilters();
    } else {
  ...
  twoLevelSelected(event, name) {
    ...
    if (name === 'nofilter') {
      this.brandsFilters.categoryID = event.value;
      this.brandsFilters.categoryLevel = 2;
      this.sendFilters();
    } else {
  ...
  threeLevelSelected(event) {
    this.brandsFilters.categoryID = event.value;
    this.brandsFilters.categoryLevel = 3;
    this.sendFilters();
  }

  signFilter(event, name) {
    if (name === 'نشان ویژه') {
      this.brandsFilters.SpecialBadge = event.checked;
      this.sendFilters();
    } else if (name === 'نشان تضمین') {
      this.brandsFilters.GuaranteeBadge = event.checked;
      this.sendFilters();
    } else if (name === 'نشان اعتماد') {
      this.brandsFilters.TrustBadge = event.checked;
      this.sendFilters();
    }
  }

  selectRegion(event) {
    this.brandsFilters.region = event.value;
    console.log(this.brandsFilters);
    this.sendFilters();
  }
